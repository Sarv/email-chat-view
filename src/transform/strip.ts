 ...
 *
 * Each family is exposed on its own (`stripSignature`, `stripQuote`,
 * `stripDisclaimer`, `stripMarkers`) because they are independently useful and
 * independently testable — and because a consumer with an unusual corpus may
 * want three of the four.
 *
 ...
 *   2. QUOTE containers, removing whole marked-up histories.
 *   3. MARKERS, on the serialized string, catching the plain-text boundaries no
 *      container marked up ("On Mon, Alice wrote:" between two bare divs).
 *   4. DISCLAIMER last, because it is the only pass that reasons about the
 *      TRAILING EDGE. Run it any earlier and a body whose tail is unmarked
 ...
 *
 * Steps 1–2 share ONE parse. Step 4 needs a second one only when step 3
